---
title: "Integrating a music library: an example with Beets"
description: "Liquidsoap's native sources can read from files and folders, but if your radio uses an important music library (more than a thousand tracks) sorting by…"
sidebar_label: "Beets"
---
...
## A source querying each next track from Beets {#a-source-querying-each-next-track-from-beets}
...
```liquidsoap title="beets-source.liq"
def beets(id, query) =
  beets_src =
    request.dynamic(
      id=id,
...
## Applying ReplayGain {#applying-replaygain}
...
```liquidsoap title="beets-amplify.liq"
def beets(id, query) =
  beets_src =
    blank.eat(
      id="#{id}_",
...
## Beets as a requests protocol {#beets-as-a-requests-protocol}
...
```liquidsoap title="beets-protocol.liq"
...
protocol.add(
  "beets",
  beets_protocol,
  syntax="same arguments as beet's random module, see \
