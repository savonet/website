...
## Node/express API {#nodeexpress-api}
...
## Redirect Icecast's pages {#redirect-icecasts-pages}
...
```liquidsoap title="harbor-metadata.liq"
meta = ref([])

# s = some source
s.on_metadata(synchronous=true, fun (m) -> meta := m)
...
```liquidsoap title="harbor-insert-metadata.liq"
