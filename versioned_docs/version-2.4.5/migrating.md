...
description: "This page lists the most common issues when migrating to a new version of Liquidsoap."
...
## From 2.3.x to 2.4.x {#from-2.3.x-to-2.4.x}
...
### Stream-related callbacks {#stream-related-callbacks}
...
### Warnings when overwriting top-level variables {#warnings-when-overwriting-top-level-variables}
...
### `null()` replaced by `null` {#null-replaced-by-null}
...
## From 2.2.x to 2.3.x {#from-2.2.x-to-2.3.x}
...
### Crossfade transitions and track marks {#crossfade-transitions-and-track-marks}
...
### `segment_name` in HLS outputs {#segment_name-in-hls-outputs}
...
### `on_air` metadata {#on_air-metadata}
...
## From 2.1.x to 2.2.x {#from-2.1.x-to-2.2.x}
...
### Icecast and Shoutcast outputs {#icecast-and-shoutcast-outputs}
...
### Default character encoding in `output.{harbor,icecast,shoutcast}` {#default-character-encoding-in-output.harboricecastshoutcast}
...
## From 2.0.x to 2.1.x {#from-2.0.x-to-2.1.x}
...
## From 1.4.x to 2.0.0 {#from-1.4.x-to-2.0.0}
...
### `auth` function in `input.harbor` {#auth-function-in-input.harbor}
...
### Type errors with lists of sources {#type-errors-with-lists-of-sources}
...
### Request-based queueing {#request-based-queueing}
...
### Returned types from output operators {#returned-types-from-output-operators}
