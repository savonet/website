...
- `add_track_mark` (of type `bool`, which defaults to `false`): Insert a track mark when the buffer becomes available again.
...
- `replay_metadata` (of type `bool`, which defaults to `false`): Replay the last metadata when the buffer becomes available again.
...
### `source.effective` {#source.effective}
