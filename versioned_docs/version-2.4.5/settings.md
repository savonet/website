...
### How much time ahead (in seconds) we should be until we let the streaming loop rest. {#how-much-time-ahead-in-seconds-we-should-be-until-we-let-the-streaming-loop-rest.}
...
### Number of sources at which a leak warning should be issued. {#number-of-sources-at-which-a-leak-warning-should-be-issued.}
...
##### Preferred codec to decode lead {#preferred-codec-to-decode-lead}

```liquidsoap
settings.decoder.ffmpeg.codecs.lead := <value>
```

###### Available codecs to decode lead {#available-codecs-to-decode-lead}
...
##### Preferred codec to decode osq {#preferred-codec-to-decode-osq}

```liquidsoap
settings.decoder.ffmpeg.codecs.osq := <value>
```

###### Available codecs to decode osq {#available-codecs-to-decode-osq}
...
##### Preferred codec to decode pdv {#preferred-codec-to-decode-pdv}

```liquidsoap
settings.decoder.ffmpeg.codecs.pdv := <value>
```

###### Available codecs to decode pdv {#available-codecs-to-decode-pdv}
...
##### Preferred codec to decode qoa {#preferred-codec-to-decode-qoa}

```liquidsoap
settings.decoder.ffmpeg.codecs.qoa := <value>
```

###### Available codecs to decode qoa {#available-codecs-to-decode-qoa}
...
##### Preferred codec to decode rtv1 {#preferred-codec-to-decode-rtv1}

```liquidsoap
settings.decoder.ffmpeg.codecs.rtv1 := <value>
```

###### Available codecs to decode rtv1 {#available-codecs-to-decode-rtv1}
...
##### Preferred codec to decode vmix {#preferred-codec-to-decode-vmix}

```liquidsoap
settings.decoder.ffmpeg.codecs.vmix := <value>
```

###### Available codecs to decode vmix {#available-codecs-to-decode-vmix}
...
##### Preferred codec to decode vvc {#preferred-codec-to-decode-vvc}

```liquidsoap
settings.decoder.ffmpeg.codecs.vvc := <value>
```

###### Available codecs to decode vvc {#available-codecs-to-decode-vvc}
...
#### Process logs through the main liquidsoap log facilities. {#process-logs-through-the-main-liquidsoap-log-facilities.}
...
#### Include metadata when computing frame checksum {#include-metadata-when-computing-frame-checksum}

```liquidsoap
settings.frame.checksum.metadata := false
```

#### Include track marks when computing frame checksum. {#include-track-marks-when-computing-frame-checksum.}
...
#### Logging level. One of: `"emergency"`, `"alert"`, `"critical"`, `"error"`, `"warning"`, `"notice"`, `"info"` or `"debug"`. {#logging-level.-one-of-emergency-alert-critical-error-warning-notice-info-or-debug.}
...
##### Exclude these metadata from automatic recoding. {#exclude-these-metadata-from-automatic-recoding.}

```liquidsoap
settings.request.metadata_decoders.recode.exclude := []
```

###### Exclude metadata listed as cover art from automatic recoding. {#exclude-metadata-listed-as-cover-art-from-automatic-recoding.}
...
### Maximum byte length for a string to be considered printable (not binary) {#maximum-byte-length-for-a-string-to-be-considered-printable-not-binary}
...
## Metadata used to store track gain normalization metadata {#metadata-used-to-store-track-gain-normalization-metadata}
...
### Metadata used to store integrated LUFS {#metadata-used-to-store-integrated-lufs}

```liquidsoap
settings.lufs.integrated_metadata := "liq_integrated_lufs"
```

### Decoding ratio used when decoding integrated LUFS from files {#decoding-ratio-used-when-decoding-integrated-lufs-from-files}
