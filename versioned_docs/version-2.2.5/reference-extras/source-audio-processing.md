...
```
(?id : string?, ?namespace : string?, ?register_server_commands : bool,
 [source(audio=pcm('a), 'b).{id? : string}]) -> source(audio=pcm('a), 'b)
where
  'b is a set of tracks to be muxed into a source and a set of internal tracks
```
...
- `id` (of type `string?`, which defaults to `null`): Force the value of the source ID.
- `namespace` (of type `string?`, which defaults to `null`): Namespace for the source commands. Uses source ID when `null`.
- `register_server_commands` (of type `bool`, which defaults to `true`): Register corresponding server commands
- `(unlabeled)` (of type `[source(audio=pcm('a), 'b).{id? : string}]
where
  'b is a set of tracks to be muxed into a source and a set of internal tracks`)
