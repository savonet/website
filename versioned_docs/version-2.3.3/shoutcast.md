...
description: "Although Liquidsoap is primarily aimed at streaming to Icecast servers (that provide much more features than Shoutcast), it is also able to stream to…"
sidebar_label: "Shoutcast output"
---
...
## Shoutcast output {#shoutcast-output}
...
```liquidsoap title="shoutcast.liq"
source = single("audiofile.ogg")

output.shoutcast(
  %mp3, host="shoutcast.example.org", port=8000, password="changeme", source
)
```

As usual, `liquidsoap -h output.shoutcast` gives you the full list of options for this operator.

## Shoutcast as relay {#shoutcast-as-relay}
