...
s =
 input.srt(
 content_type=
 "application/ffmpeg;format=s16le,ch_layout=stereo,sample_rate=48000"
 )
...
## Force a file/playlist to be played at least every XX minutes {#force-a-fileplaylist-to-be-played-at-least-every-xx-minutes}
...
## Handle special events: mix or switch {#handle-special-events-mix-or-switch}
...
## Play a short silence when transitioning out of `input.harbor` {#play-a-short-silence-when-transitioning-out-of-input.harbor}
...
## Dump a stream into segmented files {#dump-a-stream-into-segmented-files}
...
### Switch-based transitions {#switch-based-transitions}
...
### Crossfade-based transitions {#crossfade-based-transitions}
